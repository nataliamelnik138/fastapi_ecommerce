...
async def get_all_products(
        ...
        sort_by_date: SortOrder | None = Query(None, description="Сортировка по дате"),
    ...
    if sort_by_date == SortOrder.desc:
        order_criterion = desc(ProductModel.created_at)
    elif sort_by_date == SortOrder.asc:
        order_criterion = asc(ProductModel.created_at)
    else:
        # Если параметр не передан, сортируем по id по умолчанию
        order_criterion = asc(ProductModel.id)
